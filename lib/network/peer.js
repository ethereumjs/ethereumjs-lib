...
function parseHello(payload) {
  ...
  payload[3].forEach(function (p) {
    caps[p[0].toString()] = etherUtils.bufferToInt(p[1]);
  });
...
Peer.prototype.getPeers = function (cb) {
  this.once('message.peers', function () {
    this._state.gettingPeers = false;
    cb();
  });
  this._sendGetPeers();
};
...
Peer.prototype._sendGetPeers = function (cb) {
  //only ask for more peers if we have not already asked
  if (!this._state.gettingPeers) {
    this._state.gettingPeers = true;
...
/**
 * Sends peers to peer. It should only send
 * @method _sendPeers
 * @param {Array} peers
 * @param {Function} cb
 */
Peer.prototype._sendPeers = function (peers, cb) {
  if (this._state.wantPeers) {
    this._state.wantPeers = false;
 ...
 * @param {Function} cb
 */
Peer.prototype.sendNewBlock = function (block, td, cb) {
  var msg = [codes.code.newBlock, block.serialize(false), td];
  this.sendMessage(msg, cb);
};
