...
module.exports = function(opts, cb) {
  ...
  // console.log('before run: ' + toAccount.balance.toString('hex'));
  // this.cache.put(opts.to, toAccount);
  ...
  function runCode(cb2) {
    ...
    if (opts.code) {
      ...
      codeRunner.call(self, runCodeOpts, function(err, results) {
        ...
        // console.log('after run: ' + toAccount.balance.toString('hex'));
        vmResults = results;
        ...
        if (results.exceptionErr) {
          self.cache.revert();
          // var a = self.cache.get(opts.to);
          // console.log('balance after revert: ' + a.balance.toString('hex'));
