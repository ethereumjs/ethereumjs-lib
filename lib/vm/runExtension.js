...
module.exports = function(opts) {
  ...
  if (to.cmp(new BN(3)) === 1 || to.cmp(new BN(1)) === -1 ) {
    return false;
  }

  gasCost = new BN(GAS_COST[to]);
  if (opts.gas.cmp(gasCost) === -1) {
 ...
 if (to.cmp(new BN(1)) === 0) {
    ...
    if (opts.gas.cmp(new BN(gasCost + dataGas2)) === -1) {
      results.gasUsed = opts.gas;
      results.exception = 0; // 0 means VM fail (in this case because of OOG)
      return results;
    }
