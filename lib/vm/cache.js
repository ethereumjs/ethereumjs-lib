...
Cache.prototype.flush = function(cb){
  ...
  async.whilst(function(){
    ...
    if(it.value.modified){
      // console.log('key: ' + it.key + " " + new Account(it.value.val).balance.toString('hex'));
      // console.log('raw: ' +  it.value.val.toString('hex'));
    ...
    self._trie.get(new Buffer('aaaf5374fce5edbc8e2a8697c15331677e6ebf0b', 'hex'), function(err, account){
      // console.log('rawL: ' + account.toString('hex'));
      // var account  = new Account(account);
      // console.log('balance: ' + account.balance.toString('hex'));
...
Cache.prototype.revert = function(){
  ...
  // var account = this.get(new Buffer('aaaf5374fce5edbc8e2a8697c15331677e6ebf0b', 'hex'));
  // console.log('test: ' + account.balance.toString('hex'));
