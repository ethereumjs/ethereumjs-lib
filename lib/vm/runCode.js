...
module.exports = function(opts, cb) {
  ...
  function subMemUsage(offset, length) {
    ...
    var gnNewWords =new BN((newWords - wordsInMem).toString());
    gasLeft = gasLeft.sub(gnNewWords.add(gnNewWords.sqr().div(new BN(1024))));
  ...
  function checkCallMemCost(callOptions, localOpts) {
    ...
    if (gasLeft.cmp(callOptions.gas) === -1 || err === ERROR.OUT_OF_GAS || callOptions.data === ERROR.OUT_OF_GAS) {
      return ERROR.OUT_OF_GAS;
    }
  }
  ...
  var opFuncs = {
    ...
    EXP: [2,
      function(done) {
        ...
        if (exponent[0] !== 0) {
          gasLeft = gasLeft.sub(new BN(exponent.length));
        }

        stack.push(
          new Buffer(
            base.redPow(new BN(exponent)).toArray()
          )
        );
        done();
      }
    ...
    CALLDATACOPY: [3,
      function(done) {
        ...
        //sub the COPY fee
        gasLeft = gasLeft.sub(new BN(Math.ceil(dataLength / 32)));
    ...
    CODECOPY: [3,
      function(done) {
        ...
        //sub the COPY fee
        gasLeft = gasLeft.sub(new BN(Math.ceil(length / 32)));
    ...
    SSTORE: [2,
      function(done) {
        ...
        storageTrie.get(key, function(err, found) {
          ...
          if (val === '' && !found) {
            gasLeft = gasLeft.sub(new BN(100));
          } else if (val === '' && found) {
            gasRefund = gasRefund.add(new BN(100));
          } else if (val !== '' && !found) {
            gasLeft = gasLeft.sub(new BN(300));
          } else if (val !== '' && found) {
            gasLeft = gasLeft.sub(new BN(100));
          }
