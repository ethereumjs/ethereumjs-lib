 ...
 * @param opts.from {Buffer} the address that ran this code
...
module.exports = function(opts, cb) {
  ...
  var opFuncs = {
    ...
    BLOCKHASH: [1,
      function(done) {
        var hash = stack.pop();
        opts.blockchain.getBlock(hash, function(err, block){

          err = null;
          if(block){
            var diff = utils.intToBuffer(opts.block.header.number) - utils.intToBuffer(block.header.number);
            if(diff < 0 || diff >= 256){
              stack.push(new Buffer([0]));
            }else{
