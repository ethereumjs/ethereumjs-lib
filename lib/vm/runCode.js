...
module.exports = function(opts, cb) {
  var self = this,
    ...
    depth = 0, //call depth
    logs = [],
    invalidJumps = [], //a array of know bad jumps
    gasRefund = bignum(0);
  ...
  function preprocessInvalidJumps(){
    ...
    for (var i = 0; i < opts.code.length; i++){
      ...
      if(curOpCode === 'PUSH'){
        var num = opts.code[i] - 0x5f;
        for(var q = 1; q <= num; q++){
          invalidJumps[i + q] = true;
        }
        i += num;
      }

      //Cannot jump onto a jump
      if(curOpCode === 'JUMP' || curOpCode === 'JUMPI' ){
        if(lastOp === 'PUSH'){
          invalidJumps[i] = 'postPUSH';
        }else{
          invalidJumps[i] = true;
        }
      }
  ...
  function checkJump(dest){
    if(invalidJumps[dest]){
      return false;
    }

    if(invalidJumps[pc - 1] !== 'postPUSH' &&  opcodes(opts.code[dest]) !== 'JUMPDEST' ){
      return false;
    }
