  ...
  function makeCall(callOptions, localOpts, done) {
    ...
    if (depth > constants.MAX_CALL_DEPTH || bignum.fromBuffer(opts.account.balance).lt(callOptions.value)) {
      ...
      if (!callOptions.to) {
        ...
        //creating 
        console.log('//create');
        console.log(opts.address.toString('hex') + ':' + opts.account.nonce.toString('hex'));
      ...
      self.runCall(callOptions, function(err, results) {
        ...
        if(create && results.vm.exceptionErr){
          opts.account.nonce = bignum.fromBuffer(opts.account.nonce).sub(1).toBuffer();
          if(opts.address.toString('hex') ==='a94f5374fce5edbc8e2a8697c15331677e6ebf0b' && opts.account.nonce.toString('hex')  === '00' ){
            console.log("value");
