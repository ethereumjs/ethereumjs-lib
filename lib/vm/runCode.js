      ...
      SIGNEXTEND: [2,
        function(done) {
          var k = bignum.fromBuffer(stack.pop()),
            b = utils.pad256(stack.pop()),
            ONES = TT256.sub(1),
            i;

console.log('k: ', k)
console.log('old b: ', b.toString('hex'))
          if (k.le(31)) {

console.log('b[31-k]:', b[31-k])
            if (b[31-k] & 128) {
...
console.log('new b: ', b.toString('hex'))

            stack.push(b);

            // if ( b.shiftRight(k.mul(8)).and(bignum('80',16)).gt(0) ) {
            //   for (i = 31; i > k; i--) {
            //     b = b.or(ONES.shiftLeft(i));
            //   }
            //   stack.push(b.toBuffer());
            // }
