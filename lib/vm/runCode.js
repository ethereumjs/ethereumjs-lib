...
module.exports = function(opts, cb) {
  ...
  function loadAccount(done) {
    self.trie.get(opts.address, function(err, raw) {
      var toAccount = opts.account = new Account(raw);

        var c = self.cache.get(opts.address);
      ...
      // console.log('address: ' + opts.address.toString('hex'));
      // console.log('load ' + opts.account.nonce.toString('hex'));
      storageTrie.root = opts.account.stateRoot;
      done(err);
    });
  }
  ...
  var opFuncs = {
    ...
    BALANCE: [1,
      function(done) {
        ...
        self.cache.getOrLoad(address, function(err, c){
    ...
    CALL: [7,
      function(done) {
        ...
        self.cache.getOrLoad(to, function(err, c){
