...
module.exports = function (opts, cb) {
  var self = this,
    ...
    op, //the raw op code
    opcode, // the opcode add an memonic
    ...
    opFuncs = {
          ...
          self.trie.get(address, function (err, raw) {
            if (raw) {
              ...
              if (account.codeHash.length === 32) {
                self.trie.get(account.codeHash, function (err, code) {
                  stack.push(utils.intToBuffer(code));
                  done(err);
                });
              } else {
                stack.push(new Buffer([0]));
      ...
      JUMP: [1,
        function (done) {
          ...
          if (popcode !== 'JUMPDEST') {
            err = 'jump destination must have be preceded by a JUMPDEST opcode';
          }
              ...
              function (done2) {
                self.runCall(options, function (err, results) {
                  ...
                  if (results.vm) {
                    ...
                    //if a contract did not run then there is now `results.vm`
