...
module.exports = function(opts, cb) {
  ...
  function subMemUsage(offset, length) {
    ...
    wordsInMem = Math.ceil((offset + length)/ 32);
    var words = new BN(wordsInMem);
    ...
    if(cost.cmp(lastMemExpCost) === 1){
      gasLeft = gasLeft.sub(cost.sub(lastMemExpCost))
      lastMemExpCost = cost;
    }
  ...
  function makeCall(callOptions, localOpts, done) {
    ...
    if (depth > constants.MAX_CALL_DEPTH || new BN(opts.account.balance).cmp(callOptions.value) === -1) {
      ...
      self.runCall(callOptions, function(err, results) {
        ...
        //todo
        if (nonZeroCall && results.vm.exceptionErr) {
          results.gasUsed = results.gasUsed.add(new BN(2300));
        } else if (nonZeroCall) {
          console.log('warning gasUsed: ' + results.gasUsed.toString());
          console.log('warning gasProvided: ' + nonZeroCall.toString());
        }
  ...
  var opFuncs = {
    ...
    SHA3: [2,
      function(done) {
        ...
        //TODO: copy fee
        gasLeft = gasLeft.sub(new BN(Number(fees.sha3WordGas.v) * Math.ceil(length / 32)));
    ...
    LOG: [2, function(done) {
      ...
      gasLeft = gasLeft.sub(new BN(numOfTopics * Number(fees.logTopicGas.v) + memLength * Number(fees.logDataGas.v)));
    ...
    CALL: [7,
      function(done) {
        ...
        // console.log('callgas: ' + gasLimit.toString());
        if (value.cmp(new BN(0)) !== 0) {
          console.log('NON ZERO ' + value.toString() + ": " + options.gasLimit.toString());
          console.log('add: ' + opts.address.toString('hex'));
          nonZeroCall = options.gasLimit;

          gasLeft = gasLeft.sub(new BN(9000));
          gasLeft = gasLeft.add(new BN(2300));
          options.gasLimit = options.gasLimit.add(new BN(2300));
        }

        self.trie.get(to, function(err, val) {
          if (!val) {
    ...
    CALLCODE: [7,
      function(done) {
        ...
        if (value.cmp(new BN(0)) !== 0) {
          ...
          gasLeft = gasLeft.sub(new BN(9000));
          gasLeft = gasLeft.add(new BN(2300));
          options.gasLimit.add(new BN(2300));
        }
        //load the code
