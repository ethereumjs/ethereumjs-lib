...
module.exports = function(opts, cb) {
  ...
  function subMemUsage(offset, length) {
    ...
    gasLeft = gasLeft.sub( bignum(newWords - wordsInMem));
    if (gasLeft.lt(bignum(0))) {
  ...
  function makeCall(callOptions, localOpts, done) {
    ...
    if (depth > constants.MAX_CALL_DEPTH || bignum.fromBuffer(opts.account.balance).lt(callOptions.value)) {
      ...
      if (!callOptions.to) {
        opts.account.nonce = bignum.fromBuffer(opts.account.nonce).add(bignum(1)).toBuffer();
      }
      ...
      self.runCall(callOptions, function(err, results) {
        ...
        gasLeft = gasLeft.sub(bignum(results.gasUsed));
        ...
        if (!results.vm.exceptionErr) {
          ...
          if (results.vm.createdAddress) {
            opts.account.nonce = bignum.fromBuffer(opts.account.nonce).sub(1).toBuffer();
          }
          done();
        }
      });
  ...
  var opFuncs = {
    ...
    EXP: [2,
      function(done) {
        ...
        if (exponent[0] !== 0) {
          gasLeft = gasLeft.sub(bignum(exponent.length));
        }
    ...
    CODECOPY: [3,
      function(done) {
        ...
        //sub the COPY fee
        gasLeft = gasLeft.sub(bignum(Math.ceil(length / 32)));
    ...
    SSTORE: [2,
      function(done) {
        ...
        storageTrie.get(key, function(err, found) {
          ...
          if (val === '' && !found) {
            gasLeft = gasLeft.sub(bignum(100));
          } else if (val === '' && found) {
            gasRefund = gasRefund.add(bignum(100));
          } else if (val !== '' && !found) {
            gasLeft = gasLeft.sub(bignum(300));
          } else if (val !== '' && found) {
            gasLeft = gasLeft.sub(bignum(100));
          }
    ...
    LOG: [2, function(done) {
      ...
      gasLeft = gasLeft.sub(bignum(numOfTopics * 32 + memLength));
    ...
    SUICIDE: [0,
      function(done) {
        ...
        suicides.push({account:opts.address, to: suicideTo});
        done();
      }
    ]
  };
  ...
  async.whilst(function() {
    ...
    async.series([
      ...
      function(done2) {
        ...
        gasLeft = gasLeft.sub(bignum(fee));
        pc++;
