...
module.exports = function (block, root, gen, cb) {
  ...
  function processTransactions(cb) {
    ...
    var gasUsed = bignum(0),
      ...
      bloom.add(block.header.coinbase);
        ...
        function (cb3) {
          ...
          trie.put(block.header.coinbase, account.serialize(), function (err) {
            if (gen) {
              ...
              if(bloom.toString('hex') !== block.header.bloom.toString('hex')){
                err = 'invalid bloom bitvector';
              } else if (tr.state.toString('hex') !== trie.root.toString('hex')) {
